...
psd_t2 = df_t2.loc[:,keys_psd]

psd_t2 = psd_t2.to_numpy()

variance_psd = np.var(psd_t2,axis=1)
rms_psd = np.mean(psd_t2,axis=1)
...
# plt.figure()
# plt.scatter(variance_psd.reshape(-1,1), rms_psd.reshape(-1,1), c=df_t2.DamageScenario.map(colors),alpha=0.3)
...
#Outlier analysis
print(df_t2.shape)
df_t2 = df_t2[((variance_psd < high) & (variance_psd > low))]
#df_t2 = df_t2[(variance_psd < high_variance) & (variance_psd > low_variance)]
print(df_t2.shape)
...
#Save results
psd_t2.to_csv('../code_methodology/01clean_data/results/psd_t2.csv',sep=';')
metadata_t2.to_csv('../code_methodology/01clean_data/results/metadata_t2.csv',sep=';')
...
plt.figure()
plt.semilogy(psd_t2.T,color='blue',linewidth = 1)
plt.show()
