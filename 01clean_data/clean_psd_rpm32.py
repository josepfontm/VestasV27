...
psd_rpm32 = df_rpm32.loc[:,keys_psd]

psd_rpm32 = psd_rpm32.to_numpy()

variance_psd = np.var(psd_rpm32,axis=1)
rms_psd = np.mean(psd_rpm32,axis=1)
...
#Outlier analysis
print(df_rpm32.shape)
df_rpm32 = df_rpm32[((variance_psd < high) & (variance_psd > low))]
#df_rpm32 = df_rpm32[(variance_psd < high_variance) & (variance_psd > low_variance)]
print(df_rpm32.shape)

psd_rpm32 = df_rpm32.loc[:,keys_psd]
metadata_rpm32 = df_rpm32.loc[:,keys_metadata[1:]]

#Save results
psd_rpm32.to_csv('../code_methodology/01clean_data/results/psd_rpm32.csv',sep=';')
metadata_rpm32.to_csv('../code_methodology/01clean_data/results/metadata_rpm32.csv',sep=';')
...
plt.figure()
plt.semilogy(psd_rpm32.T,color='blue',linewidth = 1)
plt.show()
